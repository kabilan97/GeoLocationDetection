...
def sliding_window(x, w, compare_func):
    ...
    minimas = []
    frames = []
    for i in range(0, len(x) - wl):
        diff = compare_func(w, x[i:(i+wl)])
	minimas.append(diff)
	frames.append(i)
	# for j, val in enumerate(minimas):
	    # if diff < val and (i - frames[0]) /frame_rate > q_duration and (i - frames[1]) / frame_rate > q_duration and (i - frames[2]) / frame_rate > q_duration:
	    # 	minimas[j] = diff
	    # 	frames[j] = i
...
for video in video_list:
    ...
    if args.f == features[0]: 
        ...
        frame, score = sliding_window(x,w, euclidean_norm)	
    plt.plot(np.asarray(frame)/frame_rate,score)  
plt.legend(['K','A','B']) 
plt.xlabel('Time[s]')
plt.title('Differences for a few clips')
plt.show()
